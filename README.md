...
| `whale migrate-config` | 迁移旧配置到 `config.toml` |
...
Whale 会把本地状态存放在 `~/.whale/` 下。API key、全局 `config.toml`、会话记录、usage 日志和 MCP 配置都在本地管理。项目级配置可以放在当前仓库的 `./.whale/config.toml`。

如果你从旧版本升级，并且曾经使用过 `preferences.json` 或 `settings.json`，运行一次：
